...
def nag(input_data, input_label,val_data,val_label, layers, bias, weight, act_func_type, batch, loss, epochs, lr, anneal, gamma, expt_dir):
    ...
    for e in range(0, epochs):
        ...
        while batch_count < input_data.shape[0]:
                    ...
                    # Updating the parameters to apply nag
                    # this code is for bulk not loop by loop
                    for i in range(0, layers):
                            weight[i] = weight[i] - gamma*update_w[i]
                            bias[i] = bias[i] - gamma*update_b[i]
                    ...
                    if step % 100 == 0:
                                ...
                                train_loss = clnl.calLossNoLoop(pred_train_outputs, actual_output, act_func_type,
                                                                input_label, loss)

                                text_file_train.write("Epoch %s, Step %s, Loss: %f, Error: %f, lr: %f \n" % (
                    ...
                    if step % 100 == 0:
                        ...
                        total_val_loss[e] = clnl.calLossNoLoop(predicted_val_output, actual_val_output, act_func_type,
                                                               val_label, loss)
