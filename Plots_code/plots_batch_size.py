...
batch_size = 100
total_loss_train[2], total_loss_val[2], train_weights, train_biases = adam.adam(train_input, train_label,
                                                                                       val_input, val_label, layers,
                                                                                       biases, weights, activation,
                                                                                       batch_size,
                                                                                       loss,
                                                                                       epochs,
                                                                                        lr, anneal, gamma, expt_dir)
print("batch 100 done")

batch_size = 1000
total_loss_train[3], total_loss_val[3], train_weights, train_biases = adam.adam(train_input, train_label,
                                                                                       val_input, val_label, layers,
                                                                                       biases, weights, activation,
                                                                                       batch_size,
                                                                                       loss,
...
plt.figure(1)
plt.plot(epoch_count, total_loss_train[0].T,'r-', label='Batch_Size-1')
plt.plot(epoch_count, total_loss_train[1].T,'b-', label='Batch_Size-20')
plt.plot(epoch_count, total_loss_train[2].T,'g-', label='Batch_Size-100')
plt.plot(epoch_count, total_loss_train[3].T,'y-', label='Batch_Size-1000')
plt.legend(bbox_to_anchor=(1.05, 1), loc=4, borderaxespad=0.)
plt.xlabel('epochs',  fontsize=25)
plt.ylabel('Training Loss',  fontsize=25)
plt.title('The Plot for training loss with different Batch_Size',  fontsize=25)
...
plt.figure(2)
plt.plot(epoch_count, total_loss_val[0].T,'r-', label='Batch_Size-1')
plt.plot(epoch_count, total_loss_val[1].T,'b-', label='Batch_Size-20')
plt.plot(epoch_count, total_loss_val[2].T,'g-', label='Batch_Size-100')
plt.plot(epoch_count, total_loss_val[3].T,'y-', label='Batch_Size-1000')
plt.legend(bbox_to_anchor=(1.05, 1), loc=4, borderaxespad=0.)
plt.xlabel('epochs',  fontsize=25)
plt.ylabel('Validation Loss',  fontsize=25)
plt.title('The Plot for validation loss with different Batch_Size',  fontsize=25)
