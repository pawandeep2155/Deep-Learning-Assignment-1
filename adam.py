...
def adam(input_data, input_label, val_data, val_label, layers, bias, weight, act_func_type, batch, loss, epochs, lr, anneal, gamma, expt_dir):
    ...
    mtw, vtw, mthw, vthw = [0.0]*layers, [0.0]*layers, [0.0]*layers, [0.0]*layers
    mtb, vtb, mthb, vthb = [0.0] * layers, [0.0] * layers, [0.0] * layers, [0.0] * layers
    ...
    for e in range(0, epochs):
        ...
        while batch_count < input_data.shape[0]:
                    ...
                    for i in range(0, layers):
                            ...
                            weight[i] = weight[i] - (lr*mthw[i])/(np.sqrt(vthw[i]+epsilon))
                            bias[i] = bias[i] - (lr*mthb[i])/(np.sqrt(vthb[i]+epsilon))
                    t = t + 1
